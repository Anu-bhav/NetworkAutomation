...
for IP in IP_LIST:
    ...
    IOS_L2_S1 = DRIVER(str(IP), "anubhav", "cisco")
    IOS_L2_S1.open()

    print("Connected to " + str(IP))
    # ------------------------------------
    print("Facts - " + str(IP))
    OUTPUT = IOS_L2_S1.get_facts()
    OUTPUT = json.dumps(OUTPUT, indent=4)
    print(OUTPUT)
    # ------------------------------------
    print("Interfaces - " + str(IP))
    OUTPUT = IOS_L2_S1.get_interfaces()
    OUTPUT = json.dumps(OUTPUT, sort_keys=True, indent=4)
    print(OUTPUT)
    # ------------------------------------
    print("Interfaces IP - " + str(IP))
    OUTPUT = IOS_L2_S1.get_interfaces_ip()
    OUTPUT = json.dumps(OUTPUT, sort_keys=True, indent=4)
    print(OUTPUT)
    # ------------------------------------
    print("MAC Address Table - " + str(IP))
    OUTPUT = IOS_L2_S1.get_mac_address_table()
    OUTPUT = json.dumps(OUTPUT, sort_keys=True, indent=4)
    print(OUTPUT)
    # ------------------------------------
    print("ARP Table - " + str(IP))
    OUTPUT = IOS_L2_S1.get_arp_table()
    OUTPUT = json.dumps(OUTPUT, sort_keys=True, indent=4)
    print(OUTPUT)
    # ------------------------------------
    print("BGP Neighbors - " + str(IP))
    OUTPUT = IOS_L2_S1.get_bgp_neighbors()
    OUTPUT = json.dumps(OUTPUT, sort_keys=True, indent=4)
    print(OUTPUT)
